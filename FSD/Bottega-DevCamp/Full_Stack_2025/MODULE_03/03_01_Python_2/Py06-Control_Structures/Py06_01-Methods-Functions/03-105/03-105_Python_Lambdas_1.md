...
Now, I can call our greeting function but now I'm going to pass in our Lambda so I'm going to pass in full\_name followed by the values for it.

So if I pass in Tiffiny and then Hudgens what this is going to do is just so you can understand the mapping.

It is first going to go to the Lambda and it's going to pass in these two values and then the Lambda's going to run its process which in this case is simply formatting a string and then from there it is going to be stored in this full\_name variable and from that point we can simply pass it like any other type of variables so we can pass this Lambda directly into our greeting function and it is going to receive this name pass it into the function body and print it out.
