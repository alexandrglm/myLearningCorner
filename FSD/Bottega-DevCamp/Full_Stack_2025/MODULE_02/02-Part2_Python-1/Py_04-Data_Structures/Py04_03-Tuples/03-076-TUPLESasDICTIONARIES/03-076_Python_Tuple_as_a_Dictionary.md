...
* `(1, 'premier')` → Represents a **high-priority** package named _Premier_.
* `(1, 'mvp')` → Another **high-priority** package named _MVP_.
* `(2, 'standard')` → Represents a **standard-priority** package named _Standard_.
...
We also wanted to include some metadata into the Keys, so I'm going to create a variable here called Priority index and I'm going to place a dictionary inside of it.
...
The second package is also going to have a weight of 1 and then it's going to be called the \*_MVP package_, and then we can pass an `84`, `22`, and `24` just made-up product\
