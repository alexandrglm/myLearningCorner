...
Now you're in the market for a new home and need to compare some things first. Instantiate an object called `choice1` and set the `type` to _"house"_. It must return true. Instantiate another object called `choice2` and the `type` must be set to _"apartment"_ and that must return false.
