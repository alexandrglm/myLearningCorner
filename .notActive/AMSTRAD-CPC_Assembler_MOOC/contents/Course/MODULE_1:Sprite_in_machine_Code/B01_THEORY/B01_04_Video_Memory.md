# B01\_04\_Video\_Memory

## B1\_04: Video Memory introduction
...
Video memory stores datasets that are read by the _CRCT_ (Cathode Ray Tube Controller) to send visual information to the monitor.\
This is a universal concept in computing, though the organization and formats may vary between systems.![image](https://github.com/user-attachments/assets/69e97767-000c-43d0-85f5-1be4374a34b7)
...
The _CRCController_ chip on the motherboard constantly sends data from video memory to the monitor to create the desired objects to be displayed.
...
So, the \*\*CRCControlle&#x72;_&#x6F;ntroller_ :
...
Both the CPU and _CRCController_ share video memory. When the CPU writes to this memory, the next time the _CRCController_ reads it, the updated data is displayed.
...
The _CRCController_ refreshes the screen 50 times per second (50 Hz), redrawing the entire screen line by line.\
...
The update appears after the _CRCController_ completes a full screen refresh cycle.

***

## Use case: Understanding the Video Memory

#### _CRCController_ and Memory Interaction

* Writing specific values into video memory determines the color and position of pixels on the screen.
* The _CRCController_ decodes these values and translates them into visual information.
...
Occurs when video memory is modified while the _CRCController_ is drawing the same region.\
